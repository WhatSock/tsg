/*!
AccDC API - 3.4 for MooTools (11/19/2017)
...
self[0].innerHTML = '';
if (nData.nodeType)
self[0].appendChild(nData);
else
$(self[0]).set('html', nData);
if ( callback )
...
if (n == 'cache')
o[pL.mootoolsModVals[n]] = o[n] ? false : true;
else
o[pL.mootoolsModVals[n]] = o[n];
}
}
return o;
},
...
var s = pL.mootoolsMod(pL.extend(true, {}, pL.ajaxSettings, origSettings)),
xhr, jsonp, status, data, type = s.method.toLowerCase(), noContent = rnoContent.test(type);
...
onLoadstart: typeof s.onLoadstart === 'function' ? s.onLoadstart : undefined,
onProgress: typeof s.onProgress === 'function' ? s.onProgress : undefined,
onCancel: typeof s.onCancel === 'function' ? s.onCancel : undefined,
onException: typeof s.onException === 'function' ? s.onException : undefined,
...
if (e2 == 'load' && (ta == 'body' || ta == window || ta == document || ta == document.body))
pL(fn);
else {
if (typeof ta === 'string')
ta = pL.find(ta);
...
if (wl.loaded)
// MooTools
$(ss).set('html', '<span>&nbsp;' + wl.tabRole + '&nbsp;' + wl.tabState + '</span>');
else 
// MooTools
$(ss).set('html', '<span>&nbsp;' + wl.tabRole + '</span>');
}
}
}
...
// MooTools
$(pL.data(this, 'sra')).set('html', '<span>&nbsp;' + dc.tabRole + '</span>');
}
});
}
} else if (dc.isToggle){
...
if (!isClose)
// MooTools
$(ss).set('html', '<span>&nbsp;' + dc.toggleRole + '&nbsp;' + dc.toggleState + '</span>');
else
// MooTools
$(ss).set('html', '<span>&nbsp;' + dc.toggleRole + '</span>');
}
});
}
...
if (dc.isTab)
// MooTools
$(ss).set('html', (dc.loaded ? ('<span>&nbsp;' + dc.tabRole + '&nbsp;' + dc.tabState + '</span>') : ('<span>&nbsp;' + dc.tabRole + '</span>')));
else if (dc.isToggle)
// MooTools
$(ss).set('html', (dc.loaded ? ('<span>&nbsp;' + dc.toggleRole + '&nbsp;' + dc.toggleState + '</span>') : ('<span>&nbsp;' + dc.toggleRole + '</span>')));
}
}
});
return dc;
...
$(dc.containerDiv).set('html', source);
return dc;
},
hLoadData: '',
