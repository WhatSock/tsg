...
@end @*/
    ...
    insertAfter = function(f, s) {
      if (!f) return s;
      var ns = nextSib(f);
      if (ns) insertBefore(ns, s);
      else f.parentNode.appendChild(s);
      return s;
    },
    insertFirst = function(f, s) {
      if (!f) return s;
      var fc = firstChild(f);
      if (fc) insertBefore(fc, s);
          ...
          if ((options = arguments[i]) != null) {
            ...
            for (name in options) {
              ...
              if (
                deep &&
                copy &&
                (pL.isPlainObject(copy) || (copyIsArray = pL.isArray(copy)))
              ) {
                if (copyIsArray) {
                  copyIsArray = false;
      ...
      pL.extend({
        toDOM: function(str) {
          if (str && typeof str === "string") {
            // MooTools
            return new Element("div").set("html", str).childNodes;
          }
          return str;
        },
  ...
  (function() {
    ...
    pL.support.checkClone = fragment
      .cloneNode(true)
      ...
      if (name) {
        ...
        if (isNode) {
          try {
            ...
            if (elem.removeAttribute) elem.removeAttribute(pL.expando);
          }
  ...
  pL.event = {
      ...
      while ((type = types[i++])) {
        ...
        if (!handlers) {
          ...
          if (
            ...
            // We need to use a standard event system since the in-built MooTools binding system doesn't always work reliably
      ...
      while ((type = types[i++])) {
        ...
        if (eventType.length === 0 || (pos != null && eventType.length === 1)) {
          if (
            ...
            // Since MooTools event bindings don't always work reliably, we need to unbind the special event bindings manually
    ...
    trigger: function(event, data, elem) {
      ...
      try {
        if (
          !(elem && elem.nodeName && pL.noData[elem.nodeName.toLowerCase()])
        ) {
          if (
            elem["on" + type] &&
      ...
      if (!event.isPropagationStopped() && parent) {
        ...
        if (
          ...
          !(
            target &&
            target.nodeName &&
            pL.noData[target.nodeName.toLowerCase()]
          )
        ) {
          try {
  ...
  pL.each(
    ...
    function(i, name) {
      ...
      if (rkeyEvent.test(name)) pL.event.fixHooks[name] = pL.event.keyHooks;
  ...
  (function() {
    var Sizzle = function(selector, context, results, seed) {
      ...
      Array.each(Slick.search(context, selector), function(o, i) {
        if (inArray(o, results) === -1) results.push(o);
      });
      return results;
    };
    ...
    closest: function(selectors, context) {
      ...
      for (; i < l; i++) {
        ...
        while (
          ...
          if (
            pos ? pos.index(cur) > -1 : pL.find.matchesSelector(cur, selectors)
          ) {
            ret.push(cur);
            break;
          }
  ...
  pL.buildFragment = function(args, nodes, scripts) {
    ...
    if (
      ...
      (pL.support.checkClone || !rchecked.test(args[0]))
    ) {
      cacheable = true;
      ...
      pL.ajax({
        ...
        success: function(data, status, xhr) {
          ...
          if (selector) {
            ...
            frag = $A.createEl("div");
            // MooTools
            if (data.nodeType) frag.appendChild(data);
            else $(frag).set("html", data);
            nData = pL.find(selector, frag)[0];
          }
          if (!nData) nData = data;
          // MooTools
          pL.cleanData(self[0].getElementsByTagName("*"));
          self[0].innerHTML = "";
          if (nData.nodeType) self[0].appendChild(nData);
          else $(self[0]).set("html", nData);
          if (callback) callback.apply(self[0], [nData, status, xhr]);
  ...
  pL.extend({
    ...
    ajaxSetup: function(settings) {
      ...
      pL.mootoolsMod(pL.ajaxSettings);
    },
    ...
    mootoolsMod: function(o) {
      for (var n in o) {
        if (pL.mootoolsModVals[n]) {
          if (n == "cache") o[pL.mootoolsModVals[n]] = o[n] ? false : true;
          else o[pL.mootoolsModVals[n]] = o[n];
        }
      }
      return o;
    },
    ...
    ajax: function(origSettings) {
      var s = pL.mootoolsMod(
        ...
        status,
        data,
        type = s.method.toLowerCase(),
        noContent = rnoContent.test(type);

      s.context =
        origSettings && origSettings.context != null ? origSettings.context : s;

      // MooTools
      if (s.onSuccess && typeof s.onSuccess === "function") {
        s.success = s.onSuccess;
        s.onSuccess = null;
      }
      if (s.onComplete && typeof s.onComplete === "function") {
        s.complete = s.onComplete;
        s.onComplete = null;
      }
      if (s.onFailure && typeof s.onFailure === "function") {
        s.error = s.onFailure;
        s.onFailure = null;
      }

      var isJS = s.dataType == "script";
      ...
      if (isJS) {
              ...
              if (
                ...
                pL.handleSuccess(
                  s.dataType == "xml"
                    ...
                    : null,
                  xhr,
                  xhr && xhr.xhr ? xhr.xhr.status : null,
                  s
                );
                pL.handleComplete(
                  s.dataType == "xml"
        ...
        xhr = new Request({
          ...
          link: s.link || "chain",
          method: s.method || "get",
          emulation: typeof s.emulation === "boolean" ? s.emulation : true,
          async: typeof s.async === "boolean" ? s.async : true,
          timeout: s.timeout || 0,
          headers: s.headers || {},
          ...
          isSuccess:
            typeof s.isSuccess === "function" ? s.isSuccess : undefined,
          evalScripts:
            typeof s.evalScripts === "boolean" ? s.evalScripts : false,
          evalResponse:
            typeof s.evalResponse === "boolean" ? s.evalResponse : false,
          user: s.user || null,
          password: s.password || null,
          onLoadstart:
            typeof s.onLoadstart === "function" ? s.onLoadstart : undefined,
          onProgress:
            typeof s.onProgress === "function" ? s.onProgress : undefined,
          onCancel: typeof s.onCancel === "function" ? s.onCancel : undefined,
          onException:
            typeof s.onException === "function" ? s.onException : undefined,
          onTimeout:
            typeof s.onTimeout === "function" ? s.onTimeout : undefined,
          onSuccess: function(responseText, responseXML) {
            // success/load
            pL.handleSuccess(
              s.dataType == "xml" ? xhr.xhr.responseXML : xhr.xhr.responseText,
              xhr,
              xhr.xhr.status,
              s
            );
  ...
  (function(pL) {
    ...
    $A.getScript = function(source, callback, disableAsync) {
      ...
      if (disableAsync) pL.ajaxSetup({ async: false });
      if (isArray(source)) pL.getScripts(source, callback);
      else pL.getScript(source, callback);
      if (disableAsync) pL.ajaxSetup({ async: true });
    };
    ...
    $A.bind = function(ta2, e2, fn, data, kn) {
      ...
      if (
        ...
        if (typeof ta === "string") ta = pL.find(ta);
        if (inArray(":", e2) === -1) {
          if (typeof e2 === "string") {
            var tmp = {};
            tmp[e2] = null;
          ...
          for (var e in e2) {
            ...
            for (var x = 0; x < evs.length; x++) {
              if (evs[x]) evsa.push(evs[x].split("."));
            }
            pL(ta).each(function(i, o) {
              if (data && kn) pL.data(o, kn, data);
              var stor = pL.data(o, eventHashTS) || [];
              for (var x = 0; x < evsa.length; x++) {
                // MooTools
                var h = o;
                pL.event.add(o, evsa[x][0], e2[e] || fn);
                stor.push({
                  e: evsa[x][0],
                  h: h,
                  f: e2[e] || fn,
    ...
    $A.unbind = function(ta2, e, kn) {
      ...
      if (inArray(":", e) === -1) {
        ...
        pL(ta).each(function(i, o) {
          if (kn && typeof kn === "string") pL.removeData(o, kn);
          var stor = pL.data(o, eventHashTS) || [],
            storI = stor.length;
          for (var x = 0; x < evsa.length; x++) {
            if (stor.length) {
              for (var z = stor.length - 1; z >= 0; z--) {
                if (
                  (evsa[x][0] &&
                    evsa[x][0] == stor[z].e &&
                    !evsa[x][1] &&
                    (typeof kn !== "function" || kn === stor[z].f)) ||
                  (evsa[x][0] &&
                    evsa[x][0] == stor[z].e &&
                    evsa[x][1] &&
                    evsa[x][1] == stor[z].n &&
                  ...
                  // MooTools
                  pL.event.remove(stor[z].h, stor[z].e, stor[z].f);
                  stor.splice(z, 1);
                }
              }
            }
          }
          if (storI != stor.length) pL.data(o, eventHashTS, stor);
        });
      }
      return ta2;
    };

    $A.trigger = function(ta2, e, args, delay) {
      if (!ta2) return;
      var ta = ta2;
      if (ta.pL && ta.pL == accDCVersion) {
        ta = ta.get();
      ...
      if (inArray(":", e) === -1) {
        ...
        pL(ta).each(function(i, o) {
          for (var x = 0; x < evsa.length; x++) {
            ...
            if (delay)
              setTimeout(function() {
                pL.event.trigger(evsa[x][0], args, o);
              }, delay);
            else pL.event.trigger(evsa[x][0], args, o);
          }
        });
      }
      return ta2;
    ...
    String.prototype.announce = function announce(strm, loop, noRep, aggr) {
      ...
      if (String.announce.alertMsgs.length == 1 || loop) {
        ...
        if (
          ...
          if (aggr)
            String.announce.placeHolder2.innerHTML =
              String.announce.alertMsgs[0];
          else
    ...
    pL.accDC = function(accDCObjects, gImport, parentDC) {
      var wheel = [],
        ...
        loadAccDCObj = function(dc) {
          ...
          if (dc.showHiddenBounds) dc.accDCObj.appendChild(dc.fn.sraStart);
          dc.accDCObj.appendChild(dc.containerDiv);
          if (dc.showHiddenBounds) dc.accDCObj.appendChild(dc.fn.sraEnd);
          ...
          switch (dc.mode) {
            ...
            case 3:
              ...
              break;
            case 4:
              dc.request = pL.getScript(dc.source, function(
                source,
                textStatus
              ) {
        ...
        parseRemaining = function(dc) {
          ...
          if (dc.fn.morph && dc.fn.morphObj) {
            // MooTools
            insertAfter(dc.fn.morphObj, dc.accDCObj);
            dc.containerDiv.appendChild(dc.fn.morphObj);
            dc.fn.morph = false;
          } else if (dc.isStatic) {
            var node = null;
            if (typeof dc.isStatic === "string") node = pL.find(dc.isStatic)[0];
            else if (dc.isStatic.nodeType === 1) node = dc.isStatic;
            if (dc.append)
              // MooTools
              node.appendChild(dc.accDCObj);
            else if (dc.prepend)
              // MooTools
              insertFirst(node, dc.accDCObj);
            else {
              // MooTools
              pL.cleanData(node.getElementsByTagName("*"));
              node.innerHTML = "";
              node.appendChild(dc.accDCObj);
            }
          } else if (dc.targetObj && (!dc.returnFocus || dc.triggerObj))
            // MooTools
            insertAfter(dc.targetObj, dc.accDCObj);
          else if (dc.triggerObj)
            // MooTools
            insertAfter(dc.triggerObj, dc.accDCObj);
          else if ($A.fn.debug)
            alert(
      ...
      for (var a = 0; a < accDCObjects.length; a++) {
        var dc = {
            ...
            hSource: function(source, dc) {
              ...
              // MooTools
              if (source.nodeType) {
                dc.containerDiv.innerHTML = "";
                dc.containerDiv.appendChild(source);
              } else $(dc.containerDiv).set("html", source);
              return dc;
            },
            hLoadData: "",
                ...
                transition(
                  ...
                  },
                  {
                    ...
                    complete: function() {
                      ...
                      $A.setFocus(
                        (rft.nodeType === 1 ? rft : pL(rft).get(0)) ||
                          dc.accDCObj
                      );
