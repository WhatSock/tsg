...
(function() {
  ...
  $A.setMenu = function(
    ...
    var bfr = isInternal ? $A.getEl(path) : $A.createEl("div"),
      ...
      runAfter = function(dc) {
        ...
        (dc.mNode = $A.query(
          (config.menuTag || "ul") + "." + (config.menuClass || "menu"),
          dc.containerDiv
        )[0]),
          (ids = []);
        var offsetLeft = parseInt($A.getAttr(dc.mNode, "data-offsetleft")),
          offsetTop = parseInt($A.getAttr(dc.mNode, "data-offsettop")),
          redraw = false,
          hor = $A.getAttr(dc.mNode, "data-horizontal"),
          autoPosition = parseInt($A.getAttr(dc.mNode, "data-autoposition"));

        if (!hor) hor = config.horizontal ? "true" : "false";
        ...
        if (redraw) dc.setPosition();
        createMenu(
          (config.menuTag || "ul") + "." + (config.menuClass || "menu"),
          (config.menuTag || "ul") +
            "." +
            (config.menuClass || "menu") +
            " " +
            (config.itemTag || "li"),
          dc,
          config
          ...
          function(i, o) {
            subMenuObjects.push({
              ...
              runBefore: function(dc) {
                ...
                dc.offsetLeft =
                  typeof config.offsetLeft === "number"
                    ? config.offsetLeft
                    : typeof config.offsetLeft === "function"
                    ? config.offsetLeft(dc)
                    : !config.horizontal && dc.parent
                    ? -(dc.parent.accDCObj.offsetWidth / 2)
                    : 0;
                dc.offsetTop =
                  typeof config.offsetTop === "number"
                    ? config.offsetTop
                    : typeof config.offsetTop === "function"
                    ? config.offsetTop(dc)
                    : !config.horizontal
                    ? -dc.triggerObj.offsetHeight
                    : 0;
              },
              ...
              cssObj: {
                position:
                  (config.overrides &&
                    config.overrides.cssObj &&
                    config.overrides.cssObj.position) ||
                  "relative",
        ...
        if (ids.length) $A.setAttr(dc.mNode, "aria-owns", ids.join(" "));
        ...
        if (dc == dc.top) {
          $A.bind("body", "focusin.accmenu", function(ev) {
            if (dc.tempFocus) dc.tempFocus = null;
            else if (!("ontouchstart" in window)) dc.top.close();
          });
      ...
      runBeforeClose = function(dc) {
        ...
        if (dc.children.length) {
          for (var i = 0; i < dc.children.length; i++) dc.children[i].close();
        }
        ...
        if (dc == dc.top) $A.unbind("body", "focusin.accmenu");
      },
          ...
          [
            {
              ...
              cssObj: {
                ...
                zIndex:
                  (config.overrides &&
                    config.overrides.cssObj &&
                    config.overrides.cssObj.zIndex) ||
                  1
              },
              className: config.containerClass || "menu",
    ...
    if (isInternal) postLoad();
    // Otherwise preload HTML markup using lazy loading
    else $A.load(bfr, path, postLoad);

    if (
      config.rightClick &&
      config.rightClickText &&
      !(
        "ontouchstart" in window ||
        navigator.maxTouchPoints > 0 ||
        navigator.msMaxTouchPoints > 0
    ...
    if (typeof trapC !== "function") {
      trapC = function(ev) {
        if (!trapC.pass && (!trapCM || !trapCM.pass) && trapC.menuOpen) {
          if (
            trapC.currentMenu &&
            trapC.currentMenu.id &&
            trapC.currentMenu.loaded
          )
            trapC.currentMenu.close();
        }

        if (trapC) trapC.pass = false;

        if (trapCM) trapCM.pass = false;
      };
    ...
    $A.query(trigger, context, function(i, tgr) {
      ...
      if (rcId) addDescribedby(tgr, rcId);
        ...
        $A.bind(tgr, {
          ...
          mouseup: function(ev) {
            ...
            if (btn == 2) {
              ...
              if ($A.reg[topLvlId].loaded) $A.reg[topLvlId].close();
              else {
  ...
  var createMenu = function(parent, children, dc, config) {
      ...
      var setFocus = function() {
        ...
        items = (dc.iNodes = $A.query(children, dc.containerDiv, function(
          i,
          o
        ) {
          ...
          $A.bind(o, {
            keydown: function(ev) {
              ...
              // 37 left, 38 up, 39 right, 40 down
                ...
                if (
                  dc != dc.top &&
                  ((k == 37 && !dc.hor) || (k == 38 && dc.hor))
                )
                  dc.close();
                else if ((k == 39 && !dc.hor) || (k == 40 && dc.hor)) {
                  if (
                    $A.hasClass(this, config.folderClass || "submenu") &&
                    $A.getAttr(this, "aria-disabled") != "true"
                  )
                    $A.trigger(this, "popupsubmenu");
                ...
                if (
                  ...
                  if (!items[index] || items[index].nodeType !== 1) index = 0;

                  if (items.length) setFocus.apply(items[index]);
                }
