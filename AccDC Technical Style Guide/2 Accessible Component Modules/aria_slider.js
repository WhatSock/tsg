...
(function() {
  $A.setSlider = function(nubId, config) {
    ...
    var nub = $A.getEl(nubId),
      pNode = nub.parentNode,
      config = config || {};
    ...
    $A.morph(nub, {
      ...
      config: {
        ...
        v2i: function(o, v) {
          if (!config.vertical)
            ...
            return parseInt(
              (v / this.max) * ($A.xHeight(pNode) - $A.xHeight(o))
            );
        },
        ...
        i2v: function(o, v) {
          if (!config.vertical)
            ...
            return parseInt(
              (v / ($A.xHeight(pNode) - $A.xHeight(o))) * this.max
            );
        },
        degradeLbl: document.createTextNode(
          config.degradeLbl || "Open Manual Slider"
        )
      },
      set: function(fn) {
        var dc = this;
        ...
        setTimeout(function() {
          dc.update.apply(dc);
          dc.onDrag.apply(this, [null, null, dc, dc.config.now]);

          if (dc.dragCallback) dc.dragCallback.call(dc, dc);
        }, 1);
      },
      update: function() {
        var dc = this,
        ...
        if (dc.config.now < dc.config.min) dc.config.now = dc.config.min;
        else if (dc.config.now > dc.config.max) dc.config.now = dc.config.max;

        dc.css({
          top: config.vertical
            ? posi.top + dc.config.v2i(dc.accDCObj, dc.config.now)
            : "",
          left: !config.vertical
            ? posi.left + dc.config.v2i(dc.accDCObj, dc.config.now)
            : ""
        });
      },
      isDraggable: true,
      ...
      onDrag: function(ev, dd, dc, v, m, s) {
        var v =
          typeof v === "number" && v >= 0
            ? v
            : !config.vertical
            ? dc.config.i2v(dc.accDCObj, dc.accDCObj.offsetLeft)
            : dc.config.i2v(dc.accDCObj, dc.accDCObj.offsetTop);
        ...
        if (dc.config.past != v) {
          ...
          $A.setAttr(dc.accDCObj, {
            "aria-valuenow": v,
            "aria-valuetext":
              config.valueText && typeof config.valueText === "function"
                ? config.valueText(dc, v)
                : v
          });

          if (dc.config.manual && !m) dc.config.sel.value = v.toString();
        }
      ...
      keyDown: function(ev, dc) {
        ...
        if ((k >= 37 && k <= 40) || (k >= 33 && k <= 36)) {
          if ((k == 37 || k == 38) && dc.config.now > dc.config.min)
            ...
            dc.config.now++;
          else if (k == 36) dc.config.now = dc.config.min;
          else if (k == 35) dc.config.now = dc.config.max;
          else if (k == 34)
            dc.config.now = !config.vertical
              ? dc.config.now - dc.config.inc > dc.config.min
                ? dc.config.now - dc.config.inc
                : dc.config.min
              : dc.config.now + dc.config.inc < dc.config.max
              ? dc.config.now + dc.config.inc
              : dc.config.max;
          else if (k == 33)
            dc.config.now = !config.vertical
      ...
      runDuring: function(dc) {
        $A.setAttr(dc.accDCObj, {
          ...
          "aria-label":
            config.ariaLabel && typeof config.ariaLabel === "function"
              ? config.ariaLabel(dc)
              : dc.role,
          ...
          "aria-valuetext":
            config.valueText && typeof config.valueText === "function"
              ? config.valueText(dc, dc.config.now)
              : dc.config.now
        });
      },
      runAfter: function(dc) {
        ...
        dc.config.degrade = $A.createEl(
          ...
          dc.sraCSS,
          null,
          dc.config.degradeLbl
        );

        if (config.degradeGracefully) pNode.appendChild(dc.config.degrade);
