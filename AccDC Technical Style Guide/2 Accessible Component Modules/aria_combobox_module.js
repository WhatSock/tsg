...
(function() {
  $A.Combobox = function(sel, combobox, child) {
    ...
    var isInput =
      combobox.nodeName.toLowerCase() === "input" ||
      combobox.nodeName.toLowerCase() === "textarea"
        ? true
        : false;
    ...
    if ($A.getAttr(combobox, "role") !== "combobox") {
      alert(
        'The combobox element is missing the role="combobox" attribute, which is required.'
      );
      return null;
    }
    ...
    if (!combobox.id) combobox.id = baseId + "cb";
      ...
      [
        {
            ...
            fn: {
              update: function() {
                ...
                dc.cb.readonly = dc.cb.isInput
                  ? $A.getAttr(dc.triggerObj, "readonly")
                    ? true
                    : false
                  : true;
                dc.cb.required = dc.cb.isInput
                  ? $A.getAttr(dc.triggerObj, "required")
                    ? true
                    : false
                ...
                for (var i = 0; i < dc.cb.optionNodes.length; i++) {
                  ...
                  o.innerHTML =
                    "<a><span>" +
                    dc.cb.options[dc.cb.optionNodes[i].value].no +
                    "</span></a>";
                ...
                if (scroll && dc.cb.key) {
                  ...
                  if (pass || dc.cb.showAll || dc.cb.readonly) {
                    ...
                    for (var i = 0; i < dc.cb.names.length; i++) {
                      if (dc.cb.wordMatch) {
                        var vA = trim(dc.cb.value)
                            .toLowerCase()
                            .split(" "),
                          nA = trim(dc.cb.names[i])
                            .toLowerCase()
                            .split(" "),
                          vx = 0,
                          nx = 0;
                        ...
                        for (var z = 0; z < vA.length; z++) {
                          for (var y = 0; y < nA.length; y++) {
                            if (
                              trim(vA[z]) &&
                              trim(nA[y]) &&
                              $A.inArray(trim(vA[z]), trim(nA[y])) !== -1 &&
                              trim(vA[z]).length === trim(nA[y]).length
                            ) {
                              nx++;
                              break;
                            }
              ...
              checkValue: function(v) {
                ...
                for (var i = 0; i < dc.cb.names.length; i++) {
                  if (
                    trim(v) &&
                    trim(v.toLowerCase()) === trim(dc.cb.names[i].toLowerCase())
                  ) {
                    return i;
                  }
                }
                    ...
                    if (dc.cb.readonly && dc.loaded && (k === 37 || k === 39)) {
                      ...
                      dc.cb.sIndex =
                        ...
                        dc.cb.sel.selectedIndex >= 0
                          ? dc.cb.sel.selectedIndex
                          : 0;
                      if (typeof dc.cb.sIndex !== "number") dc.cb.sIndex = 0;
                      dc.cb.activeDescendant = true;
                      that.open();
                      ev.preventDefault();
                    } else if (
          ...
          runBefore: function(dc) {
            ...
            $A.find("*", function(dc) {
              if (dc.comboboxControl && dc.loaded) dc.close();
            });
          },
    ...
    that.clearAll = function() {
      ...
      if (dc.cb.isInput) dc.triggerObj.value = "";
        ...
        if (!dc.cb.multiple) dc.cb.child.innerHTML = "";
