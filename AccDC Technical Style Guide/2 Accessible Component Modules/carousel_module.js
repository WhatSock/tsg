	...
	$A.setCarousel = function(container, path, autoStart, config){
		...
		$A.load(tmp, path, function(data){
			...
			var attrs =
				...
				' '), vals = {}, d = xml.documentElement,
				forward = d.attributes.getNamedItem('forward').nodeValue == 'yes' ? true : false,
				cycle = d.attributes.getNamedItem('cycle').nodeValue == 'yes' ? true : false,
				showGroup = d.attributes.getNamedItem('showGroup').nodeValue == 'yes' ? true : false,
				groupPosTop = d.attributes.getNamedItem('groupPosTop').nodeValue == 'yes' ? true : false,
				isGrouped = d.attributes.getNamedItem('isGrouped').nodeValue == 'yes' ? true : false;

			for (var i = 0; i < attrs.length; i++)
							vals[attrs[i]] = d.attributes.getNamedItem(attrs[i]).nodeValue;
			track.length = 0;
			$A.query('carousel > *', xml, function(i, n){
				...
				if (!isGrouped){
					...
					announce[bId + i] = n.attributes.getNamedItem('announce').nodeValue;
				}
					...
					$A.query('*', n, function(j, m){
						...
						announce[bId + i + j] = m.attributes.getNamedItem('announce').nodeValue;
					});
				}
			});
							...
							runAfter: function(dc){
								...
								dc.nSlide = function(ev, auto){
									...
									if (handlers.btnNext && typeof handlers.btnNext === 'function' && dc.accDCObj)
										ret = handlers.btnNext.apply(this, [ev, dc]);
								...
								dc.nGroup = function(ev){
									...
									if (handlers.btnNextG && typeof handlers.btnNextG === 'function')
										ret = handlers.btnNextG.apply(this, [ev, dc]);
