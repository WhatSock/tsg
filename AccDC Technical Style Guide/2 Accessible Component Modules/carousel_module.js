...
(function() {
  $A.setCarousel = function(container, path, autoStart, config) {
    // config variables
    var config = config || {},
      lNavCls = config.lNavCls || "lNav",
      rNavCls = config.rNavCls || "rNav",
      contentCls = config.contentCls || "centerContent",
      btnTag = config.btnTag || "button",
      btnCls = config.btnCls || "navButton",
      btnSlideCls = config.btnSlideCls || "navSlideButton",
      btnGroupCls = config.btnGroupCls || "navGroupButton",
      groupNameCls = config.groupNameCls || "groupName",
      ariaLevel = config.ariaLevel || 3,
      // Internal variables
      xml = null,
      track = {},
      bId = container.id || "l" + $A.genId(),
      announce = {},
      tmp = $A.createEl("div"),
    ...
    $A.load(tmp, path, function(data) {
      xml = str2xml(data);
      var attrs = "role height width className prevTitle nextTitle slideName groupName btnPText btnNText btnPGText btnNGText btnPAccesskey btnNAccesskey btnPGAccesskey btnNGAccesskey direction timer animDelay hiddenMsg".split(
          " "
        ),
        vals = {},
        d = xml.documentElement,
        forward =
          d.attributes.getNamedItem("forward").value == "yes" ? true : false,
        cycle =
          d.attributes.getNamedItem("cycle").value == "yes" ? true : false,
        showGroup =
          d.attributes.getNamedItem("showGroup").value == "yes" ? true : false,
        groupPosTop =
          d.attributes.getNamedItem("groupPosTop").value == "yes"
            ? true
            : false,
        isGrouped =
          d.attributes.getNamedItem("isGrouped").value == "yes" ? true : false;
      ...
      var lDiv = $A.createEl("div", null, null, lNavCls),
        rDiv = $A.createEl("div", null, null, rNavCls),
        cDiv = $A.createEl("div", null, null, contentCls),
        btnAttrs = {};

      if (btnTag == "a") btnAttrs.href = "#";

      if (btnTag != "a" && btnTag != "button") btnAttrs.tabindex = "0";

      if (btnTag != "button") btnAttrs.role = "button";
      var btnP = $A.createEl(
          btnTag,
          btnAttrs,
          null,
        ...
        btnNG = $A.createEl(btnTag, btnAttrs, null, btnCls + " " + btnGroupCls),
        pDiv = $A.createEl(
          "div",
          null,
          {
      ...
      btnP.innerHTML =
        config.prevBtnInsertedTag ||
        '<span aria-hidden="true">' + vals.btnPText + "</span>";
      ...
      btnN.innerHTML =
        config.nextBtnInsertedTag ||
        '<span aria-hidden="true">' + vals.btnNText + "</span>";
      ...
      if (isGrouped) {
        ...
        btnPG.innerHTML =
          config.prevGroupBtnInsertedTag ||
          '<span aria-hidden="true">' + vals.btnPGText + "</span>";
        ...
        btnNG.innerHTML =
          config.nextGroupBtnInsertedTag ||
          '<span aria-hidden="true">' + vals.btnNGText + "</span>";
      ...
      var renderFn =
        config.renderFn && typeof config.renderFn === "function"
          ? config.renderFn
          : function(
              pDiv,
              lDiv,
      ...
      $A([
        {
          ...
          runOnceBefore: function(dc) {
            ...
            if (groupPosTop) dc.gdc.autoPosition = 1;
            else dc.gdc.autoPosition = 5;
          },
          ...
          runAfter: function(dc) {
              ...
              if (n.toString() === "0" || n > 0) {
                if (!isGrouped) {
                  ...
                  objs.push({
                    ...
                    autoStart:
                      autoStart.toString() === n.toString() ? true : false,
                    source: track[n]
                  });
                  ...
                  if (!autoStart || typeof autoStart !== "string") aS = [0, 0];
                  else aS = autoStart.split(",");
                  ...
                  for (var m in track[n]) {
                    if (m.toString() === "0" || m > 0) {
                      objs.push({
                        ...
                        autoStart:
                          aS[0].toString() === n.toString() &&
                          aS[1].toString() === m.toString()
                            ? true
                            : false,
                        source: track[n][m]
                      });
            ...
            $A(dc, objs, {
              ...
              runAfter: function(dc) {
                ...
                if (dc.parent.cur) dc.parent.old = dc.parent.cur;
                dc.parent.cur = dc;
                ...
                if (dc.parent.firstLoaded) {
                  ...
                  if (vals.direction == "lr")
                    o.left = !dc.parent.dirFlag
                      ? o.left + bDiv.offsetWidth
                      : o.left - bDiv.offsetWidth;
                  else if (vals.direction == "tb")
                    o.top = !dc.parent.dirFlag
                      ? o.top + bDiv.offsetHeight
                      : o.top - bDiv.offsetHeight;

                  if (isGrouped && showGroup && (paused || !vals.timer))
                    $A.announce(dc.parent.gdc.groupName);

                  if (paused || !vals.timer) $A.announce(announce[dc.id]);
                  loading = true;
                  $A.setAttr(dc.parent.old.accDCObj, "aria-hidden", "true");
                  $A.setAttr(dc.accDCObj, "aria-hidden", "false");
                  animate(
                    dc.accDCObj,
                    dc.parent.old.accDCObj,
                    {
                      top: bDiv.offsetTop + bDiv.clientTop,
                      left: bDiv.offsetLeft + bDiv.clientLeft
                    },
                    o,
                    {
                      ...
                      complete: function() {
                        if (
                          ...
                          if (
                            handlers.complete &&
                            typeof handlers.complete === "function"
                          )
            ...
            dc.pSlide = function(ev, auto) {
              ...
              if (
                handlers.btnPrev &&
                typeof handlers.btnPrev === "function" &&
                dc.accDCObj
              )
              ...
              if (dc) {
                if (!isGrouped) {
                  if (dc.cur.slideVal > 0) id = bId + (dc.cur.slideVal - 1);
                  else if (cycle) id = bId + (dc.cur.slideMax - 1);
                  else return false;
                } else {
                  var g = dc.cur.groupVal,
                    s = dc.cur.slideVal;

                  if (g < 1 && s < 1 && !cycle) return false;
                  else if (s < 1) {
              ...
              if (ndc && ndc.id) ndc.open();
            };

            dc.enableAuto = function(v) {
              stop = v ? false : true;
              dc.isStopped = stop;

              if (stop) clearTimeout(dc.inc);
              else dc.setInc();

              if (
                handlers.stopStateChange &&
                typeof handlers.stopStateChange === "function"
              )
            ...
            dc.setSlide = function(slideNum, groupNum) {
              var isSN = !isNaN(slideNum) && slideNum >= 0,
                isGN = !isNaN(groupNum) && groupNum >= 0;

              if (isGN && isSN)
                dc.nGroup.apply(this, [null, slideNum, groupNum]);
              else if (isSN) dc.nSlide.apply(this, [null, null, slideNum]);
            };
            ...
            dc.nSlide = function(ev, auto, slideNum) {
              ...
              if (
                handlers.btnNext &&
                typeof handlers.btnNext === "function" &&
                dc.accDCObj
              )
              ...
              if (dc) {
                if (!isGrouped) {
                  if (!isNaN(slideNum) && slideNum >= 0) id = bId + slideNum;
                  else if (dc.cur.slideVal < dc.cur.slideMax - 1)
                    id = bId + (dc.cur.slideVal + 1);
                  else if (cycle) id = bId + 0;
                  else return false;
                } else {
                  var g = dc.cur.groupVal,
                    s = dc.cur.slideVal;
            ...
            dc.nGroup = function(ev, slideNum, groupNum) {
              if (loading) return;
              ...
              var id = "",
                ...
                sgSet =
                  !isNaN(slideNum) &&
                  slideNum >= 0 &&
                  (!isNaN(groupNum) && groupNum >= 0);

              if (g >= dc.cur.groupMax - 1 && !cycle && !sgSet) return false;
              else {
                if (sgSet) g = groupNum;
                else g = g < dc.cur.groupMax - 1 ? g + 1 : 0;
                id = bId + g + (sgSet ? slideNum : 0);
            ...
            if (vals.timer > 0) {
              ...
              else $A.setAttr(dc.fn.sraStart, "aria-hidden", "true");
            }

            if (config.pauseRotation) dc.enableAuto(false);
          },
          ...
          pauseRotation: config.pauseRotation || false,
          setInc: function() {
            var dc = this;
            ...
            if (dc.id) {
              ...
              if (vals.timer > 0)
                dc.inc = setTimeout(function() {
                  ...
                  if (forward) dc.nSlide(null, true);
                  else dc.pSlide(null, true);
                }, vals.timer);
            }
          },
