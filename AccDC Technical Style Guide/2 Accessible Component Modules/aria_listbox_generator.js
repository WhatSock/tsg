...
(function() {
  $A.Listbox = function(list, config) {
    var config = config || {},
      grabInstruct = config.grabInstruct || "Press Space to grab",
      dropInstruct = config.dropInstruct || "Press Space to drop",
      grabMsg = config.grabMsg || "Grabbed",
      dropMsg = config.dropMsg || "Moved",
      cancelDrop = config.cancelDrop || "Grab canceled",
      isArray = function(v) {
        return (
          v &&
          typeof v === "object" &&
      ...
      list = typeof list === "string" ? $A.getEl(list) : list,
      that = this;
    ...
    var track = {},
      ...
      select = function(i, f) {
        ...
        if (that.options[i]) {
          ...
          if (f) that.options[i].focus();

          if (!config.isMultiselect) selected = that.options[i].id;
      ...
      drop = function(c) {
        ...
        if (!c) {
          list.insertBefore(
            that.options[track[grabbed]].parentNode,
            that.options[that.index].parentNode
          );
          setOptions(true);
          $A.announce(dropMsg);
        } else $A.announce(cancelDrop);
        var g = grabbed;
        grabbed = "";
        select(c ? that.index : track[g], c ? false : true);
      },
      add = function(o) {
        if (!that.options.length) that.empty = true;

        if (o) {
          if (grabbed) drop.apply(this, [true]);

          var isA = isArray(o),
            a = isA ? o : [o],
            pLength = that.options.length;
          ...
          for (var b = 0; b < a.length; b++) {
            ...
            if (config.isSortable || config.isMultiselect)
              $A.setAttr(
                o,
                config.isSortable ? "aria-grabbed" : "aria-selected",
                "false"
              );
          ...
          if (pLength < that.options.length) select(pLength);
        }
      },
      rem = function(i, s) {
        if (that.options.length) {
          ...
          if (that.index > max) that.index = max;
      ...
      setBindings = function(o) {
        if (!o) {
          $A.unbind(list, "focusin.arialistbox");
        ...
        $A.unbind(o || "#" + list.id + " > li > a", "click keydown");
        ...
        $A.bind(o || "#" + list.id + " > li > a", {
          ...
          "keydown.arialistbox": function(ev) {
            ...
            if (k == 13) {
              ...
              if (that.index < max) select(that.index + 1, true);
            } else if (k == 35) {
              ev.preventDefault();
              ev.stopPropagation();
              ...
              if (that.index != g) select(g > 0 ? g : 0, true);
            } else if (k == 34) {
              ev.preventDefault();
              ev.stopPropagation();
              ...
              if (that.index != g) select(g < max ? g : max, true);
      ...
      setOptions = function(s) {
        ...
        that.options = $A.query("#" + list.id + " > li > a", function(i, o) {
          ...
          if (config.isMultiselect) toggle[o.id] = false;
          max = i;
        ...
        if (ids.length) $A.setAttr(list, "aria-owns", ids.join(" "));
        ...
        if (
          that.options.length === 1 ||
          (that.empty && that.options.length > 0)
        )
          select(0);
        that.empty = false;
      };

    if (config.isMultiselect) $A.setAttr(list, "aria-multiselectable", "true");

    if (config.label) $A.setAttr(list, "aria-label", config.label);
    setOptions();
    setBindings();
    that.val = function(i) {
      if (typeof i === "number") select(i);
      else if (i && typeof i === "string") select(track[i]);
      else if (i && isArray(i) && config.isMultiselect) {
        var inI = function(id) {
          for (var j = 0; j < i.length; j++) {
            if (i[j] == id) return true;
          }
        ...
        for (var k = 0; k < that.options.length; k++) {
          if (
            (inI(that.options[k].id) &&
              $A.getAttr(that.options[k], "aria-selected") != "true") ||
            (!inI(that.options[k].id) &&
              $A.getAttr(that.options[k], "aria-selected") != "false") ||
            (!i.length &&
              $A.getAttr(that.options[k], "aria-selected") != "false")
          )
    ...
    that.rem = function(i) {
      ...
      for (var b = 0; b < a.length; b++)
        r.push(
          !a[b]
            ? null
            : rem(a[b] && typeof a[b] === "string" ? track[a[b]] : a[b], true)
        );
      return isA ? r : r[0];
    };
