...
(function() {
  $A.setTree = function(config) {
    var config = config || {},
      treeTitle = config.title,
      insertionPoint = config.container,
      tree = config.treeTag || "ul",
      treeClass = config.treeClass || "branch",
      dividerTag = config.dividerTag || "li",
      treeitem = config.treeItemTag || "a",
      treeitemClass = config.treeItemClass || "leaf",
      selCls = config.selectClass || "selected",
      cur = (xmlDoc = null),
      ...
      createTree = function(dc) {
        ...
        for (var i = 0; i < dc.xmlNodes.length; i++) {
          ...
          if (!(n.nodeType !== 1 || n.nodeName == "#text")) {
            ...
            if (d.nodeName.toLowerCase() == "a") $A.setAttr(d, "href", "#");
            ...
            for (var j = 0; j < n.childNodes.length; j++) {
              if (
                !(
                  n.childNodes[j].nodeType !== 1 ||
                  n.childNodes[j].nodeName == "#text"
                )
              )
                isBranch = true;
            }

            if (isBranch) $A.addClass(d, treeClass);
            else $A.addClass(d, treeitemClass);
            d.innerHTML =
              "<span>" + n.attributes.getNamedItem("name").value + "</span>";
      ...
      createARIATree = function(dc, folderClass) {
        var folderClass = folderClass || "folder",
          parent = dc.tree.node,
          children = dc.tree.childNodes;
        ...
        for (var i = 0; i < children.length; i++) {
          ...
          $A.bind(l, {
            ...
            clicktreeitem: function(ev) {
              if (!$A.hasClass(this, selCls)) $A.trigger(this, "focustreeitem");
              else if (
                $A.hasClass(this, treeClass) &&
                $A.getAttr(this, "aria-expanded") == "false"
              ) {
                $A.setAttr(this, "aria-expanded", "true");
                $A.reg[this.id].open();
              } else if (
                $A.hasClass(this, treeClass) &&
                $A.getAttr(this, "aria-expanded") == "true"
              ) {
            ...
            keydown: function(ev) {
              ...
              // 35 end, 36 home, 37 left, 38 up, 39 right, 40 down
                ...
                if (k == 35) {
                  ...
                  while (pdc.parent) pdc = pdc.parent;
                  var cn = pdc.tree.childNodes;

                  while (
                    $A.hasClass(cn[cn.length - 1], treeClass) &&
                    $A.getAttr(cn[cn.length - 1], "aria-expanded") == "true"
                  )
                  ...
                  if (
                    ...
                    var cn = dc.parent.tree.childNodes,
                      go = null;
                    ...
                    if (go) $A.trigger(go, "focustreeitem");
                  } else if (
                    $A.hasClass(this, treeClass) &&
                    $A.getAttr(this, "aria-expanded") == "true"
                  ) {
                  ...
                  } else $A.trigger(this, "clicktreeitem");
                } else if (k == 38) {
                  if (dc.parent && this == children[0]) {
                    ...
                    while (
                      $A.hasClass(t, treeClass) &&
                      $A.getAttr(t, "aria-expanded") == "true"
                    ) {
                      var cn = $A.reg[t.id].tree.childNodes;
                      t = cn[cn.length - 1];
                    }

                    if (t) $A.trigger(t, "focustreeitem");
                  }
                } else if (k == 40) {
                  if (
                    $A.hasClass(this, treeClass) &&
    ...
    if (config.overrides) $A.internal.extend(overrides, config.overrides);
