  ...
  $A.setFormFields = function(form, fields, submitFn) {
        ...
        [
          {
            id: id,
            role:
              (f.overrides && f.overrides.role) ||
              (f.helpText ? "help" : "error"),
            ...
            autoPosition: isNaN(f.overrides && f.overrides.autoPosition)
              ? 3
              : f.overrides.autoPosition,
            offsetLeft: isNaN(f.overrides && f.overrides.offsetLeft)
              ? 10
              : f.overrides.offsetLeft,
            cssObj: {
              position: "absolute"
            },
            ...
            runAfter: function(dc) {
              if (!f.togglePassed)
                $A.addClass(
                  dc.classObj || dc.targetObj || dc.triggerObj,
                  f.toggleClass || "validateError"
                );
      ...
      $A.bind(o, {
        ...
        focusout: function(ev) {
          ...
          if (f.helpText) {
            ...
            if (!track[id]) {
              if (!f.hideError) dc.open();
              else dc.runAfter.apply(dc, [dc]);
            } else {
              if (!f.hideError) dc.close();
              else dc.runAfterClose.apply(dc, [dc]);
            }
          }
        }
      });
    ...
    $A.bind(form, "submit", function(ev) {
      ...
      if (v.length) {
        ...
        if (!fields[v[0]].helpText && !fields[v[0]].hideError) gdc.open();
        gdc.triggerObj.focus();
        ev.preventDefault();
      } else if (submitFn && typeof submitFn === "function") submitFn.apply(this, [ev, rFields]);
    });
  };
})();
