...
(function(pL){
	var baseId = 'grid' + $A.genId(), gridInc = 1;

	$A.DataGrid = function(container){
		var that = this, container = typeof container === 'string' ? $A.getEl(container) : container, isArray = function(v){
						...
						page:
										...
										row:
														...
														build: function(rowObject){
															...
															rowObject.rowNode = $A.createEl('tr',
																			{
																			role: 'row',
																			id: bid
																			}, null, config.gridRowClass);
															...
															for (id in rowObject.cells){
																...
																rowObject.cells[id].cellNode = $A.createEl('td',
																				{
																				id: cid
																				}, null, config.gridCellClass);

																config.page.row.bindCell(rowObject.cells[id], rowObject.cells[id].cellNode, rowObject,
																	rowObject.rowNode);
																rowObject.cells[id].cellNodeA = $A.createEl('strong',
																				{
																				id: cid + 'a'
																				}, null,
																	rowObject.cells[id].type == 'toggle' ? config.gridCellToggleClass : config.gridCellLinkClass);

																rowObject.cells[id].cellNodeS1 = $A.createEl('span');
																rowObject.cells[id].cellNodeS2 = $A.createEl('span', null, $A.sraCSS);
																rowObject.cells[id].cellNodeS3 = $A.createEl('span', null, $A.sraCSS);
																rowObject.cells[id].cellNode.appendChild(rowObject.cells[id].cellNodeA);
																rowObject.cells[id].cellNodeA.appendChild(rowObject.cells[id].cellNodeS1);
																rowObject.cells[id].cellNodeA.appendChild(rowObject.cells[id].cellNodeS2);
																rowObject.cells[id].cellNodeA.appendChild(rowObject.cells[id].cellNodeS3);
														...
														resetPos: function(cellNode){
															...
															config.page.row.trackPos.row = $A.inArray(d.rowId, config.page.rendered) || 0;
														...
														bindCell: function(cellObject, cellNode, rowObject, rowNode){
															...
															var trigger = function(){
																...
																if (config.edit && !cellObject.readonly){
																	if (cellObject.type == 'toggle'){
																		config.page.row.changed(cellNode, cellObject, cellObject.value ? false : true);
																	}
																		...
																		$A.css(config.page.row.editFieldDiv, config.page.row.cellOffset(cellObject));
																...
																if (k == 33){
																	if (pressed.alt)
																		...
																		config.page.prev(cellObject, rowObject);
																}
																	...
																	var move = false, rowMax = config.page.rendered.length - 1, colMax = config.colHeaders.map.length - 1;

																	if (k == 35 && pressed.ctrl && config.page.current < config.page.total){
																		config.page.last(cellObject, rowObject, true);
																		return;
																	}

																	else if (k == 35 && pressed.ctrl && config.page.current == config.page.total){
																		config.page.row.trackPos.row = rowMax;
																		move = true;
																	}

																	else if (k == 35 && config.page.row.trackPos.col < colMax){
																		config.page.row.trackPos.col = colMax;
																		move = true;
																	}

																	else if (k == 36 && pressed.ctrl && config.page.current > 1){
																		config.page.first(cellObject, rowObject);
																		return;
																	}
																	...
																	else if (k == 38 && config.page.row.trackPos.row > 0 && pressed.alt){
																		config.page.row.trackPos.row = 0;
																		move = true;
																	}
																	...
																	else if (k == 39 && config.page.row.trackPos.col < colMax){
																		config.page.row.trackPos.col += 1;
																		move = true;
																	}

																	else if (k == 40 && config.page.row.trackPos.row < rowMax && pressed.alt){
																		config.page.row.trackPos.row = rowMax;
																		move = true;
																	}

																	else if (k == 40 && config.page.row.trackPos.row < rowMax){
																		config.page.row.trackPos.row += 1;
																		move = true;
																	}
																	...
																	if (move){
																		config.page.row.move(
																						{
																						rowId: config.page.rendered[config.page.row.trackPos.row],
																						cellId: config.colHeaders.map[config.page.row.trackPos.col].id
																						});
																	}
																}
															};
															...
															$A.bind(cellNode,
																			...
																			'keydown.gridcontrol': function(ev){
																				...
																				if ((k >= 33 && k <= 40) || k == 13 || k == 32 || k == 46 || k == 65){
																					cellObject = getCellObject(this);
																					...
																					if (k == 13 || k == 32){
																						if (k == 32 && pressed.ctrl){
																							...
																							for (var i = 0; i < config.colHeaders.map.length; i++){
																								...
																								str += $A.getText(cellObject.rowObject.cells[o.id].cellNode) + ', ';
																							}
																							$A.announce(str);
																						}
																					...
																					else if (k == 65 && pressed.ctrl && config.page.row.selectEnabled){
																			...
																			'focus.gridcontrol': function(ev){
																				...
																				$A.addClass(cellObject.rowObject.rowNode, config.gridRowFocusedClass);

																				if (this != config.page.row.focusedCell)
																					config.page.row.move(config.page.row.resetPos(this), true);
																			},
																			'blur.gridcontrol': function(ev){
																				$A.query('tr.' + config.gridRowFocusedClass + ', td.' + config.gridCellFocusedClass,
																					config.dc.containerDiv, function(i, o){
																					$A.remClass(o, config.gridRowFocusedClass + ' ' + config.gridCellFocusedClass);
																				});
																			}
																			});
														},
														clean: function(rowObject, keepData){
															...
															if (!keepData)
																pL.removeData(rowObject.rowNode, 'rowdata');
														...
														changed: function(cellNode, cellObject, val){
															...
															if (ret){
																config.page.row.update(cellObject.rowObject.id, cellObject.id, val);
															}
														},
														selectEnabled: false,
														...
														select: function(cellObject, sv){
															...
															if (ret){
																...
																for (var i = config.page.row.selected.length - 1; i >= 0; i--){
																	if (config.page.row.selected[i] === cellObject.rowObject.id && !cellObject.rowObject.selected)
																		config.page.row.selected.splice(i, 1);

																	else if (config.page.row.selected[i] === cellObject.rowObject.id && cellObject.rowObject.selected)
																		found = true;
																}

																if (!found && cellObject.rowObject.selected)
																	config.page.row.selected.push(cellObject.rowObject.id);
																var isR = $A.inArray(cellObject.rowObject.id, config.page.rendered) !== -1;
																...
																if (isR){
																	...
																	if (cellObject.rowObject.selected){
																		$A.addClass(cellObject.rowObject.rowNode, config.page.row.selectClass);
																		altTxt = config.page.row.selectState ? config.page.row.selectState + '. ' : '';
																	}

																	else{
																		$A.remClass(cellObject.rowObject.rowNode, config.page.row.selectClass);
																	}
																	altTxt += config.page.row.dblClickTitle;
																	$A.setAttr(cellObject.rowObject.rowNode, 'title', altTxt);

																	if (config.page.row.ariaSelect)
																		$A.setAttr(cellObject.rowObject.rowNode, 'aria-selected',
																			cellObject.rowObject.selected ? 'true' : 'false');

																	for (var i = 0; i < config.colHeaders.map.length; i++){
																		if (config.page.row.ariaSelect)
																			$A.setAttr(cellObject.rowObject.cells[config.colHeaders.map[i].id].cellNode, 'aria-selected',
																				cellObject.rowObject.selected ? 'true' : 'false');

																		else
																			cellObject.rowObject.cells[config.colHeaders.map[i].id].cellNodeS3.innerHTML
																				= cellObject.rowObject.selected
																					? '<span>&nbsp;' + formatStr(config.page.row.selectState) + '</span>' : '';
																	}
																}
															}
														},
														unselectAll: function(){
															for (var i = config.page.row.selected.length - 1; i >= 0; i--){
																...
																config.page.row.select(config.page.row.collection[rowId].cells[config.colHeaders.map[0].id], false);
																config.page.row.selected.splice(i, 1);
															}
														},
														selectAll: function(){
															if (config.page.row.selectEnabled && config.page.row.multiSelect){
																if (!config.page.row.preserveSelect)
																	config.page.row.unselectAll();

																for (var i = 0; i < config.page.rendered.length; i++){
																	config.page.row.select(
																		config.page.row.collection[config.page.rendered[i]].cells[config.colHeaders.map[0].id], true);
																}
															}
														},
														deleteRow:
																		...
																		callback: function(ids){
																			...
																			var rt = config.page.row.deleteRow.callback.runBefore
																				&& typeof config.page.row.deleteRow.callback.runBefore === 'function'
																					? config.page.row.deleteRow.callback.runBefore.apply(that, [ids
																						&& ids.length ? ids : config.page.row.selected]) : true;
																			...
																			if (ids && ids.length){
																				...
																				for (var i = 0; i < ids.length; i++){
																					if (config.page.row.collection[ids[i]] && config.page.row.collection[ids[i]].cells)
																			...
																			for (var i = config.page.row.selected.length - 1; i >= 0; i--){
																				var rowObject = config.page.row.collection[config.page.row.selected[i]],
																					pos = $A.inArray(rowObject.id, config.page.row.collectionMap);
																				...
																				if (pos !== -1){
																					...
																					if (ret){
																						...
																						if (isR !== -1)
																							config.page.rendered.splice(isR, 1);
																						config.page.row.selected.splice(i, 1);
																			...
																			if (si != config.page.row.selected.length){
																				...
																				if (config.page.row.deleteRow.callback.runAfter
																					&& typeof config.page.row.deleteRow.callback.runAfter === 'function')
																					config.page.row.deleteRow.callback.runAfter.apply(that, [deleted]);
																				return true;
																			}
																			return false;
																		}
														...
														update: function(rowId, cellId, val, silent){
															if (!config.page.row.collection[rowId] || !config.page.row.collection[rowId].cells[cellId])
																return;
															var cellObject = config.page.row.collection[rowId].cells[cellId], type = cellObject.type;
															config.page.row.collection[rowId].cells[cellId].value = type == 'toggle' ? (val ? true : false) : val;
															...
															if (inA !== -1){
																if (type == 'toggle'){
																	...
																	cellObject.cellNodeS1.innerHTML = '<span>' + formatStr(val) + '</span>';
																}
														...
														move: function(o, s){
															...
															var oldCell = config.page.row.focusedCell,
																newCell = config.page.row.collection[o.rowId].cells[o.cellId].cellNode;
										...
										total: 1,
										sync: function(n){
											var prevC = config.page.current, prevT = config.page.total;
											...
											if (!config.page.row.max || !config.page.row.collectionMap.length){
												...
												var rMax = config.page.row.max, tRows = config.page.row.collectionMap.length;
												...
												if (rMax >= tRows)
													...
													if (config.page.current > p)
														config.page.current = p;
												}
											}
											...
											if (n){
												if (n > config.page.row.total)
													n = config.page.row.total;
												var g = (n - 1) * config.page.row.max, l = g + config.page.row.max;
										...
										removeRendered: function(){
											...
											for (var i = 0; i < config.page.rendered.length; i++){
												var id = config.page.rendered[i], rowObject = config.page.row.collection[id];
												...
												for (var n in rowObject.cells){
													if (rowObject.cells[n].cellNode.parentNode)
														rowObject.cells[n].cellNode.parentNode.removeChild(rowObject.cells[n].cellNode);
												}
										...
										render: function(ids){
											...
											if (config.page.rendered.length)
												config.page.removeRendered();
											...
											for (var h = 0; h < ids.length; h++){
												gridInc++;
												var tr = config.page.row.collection[ids[h]].rowNode, colIndex = 1,
												...
												$A.setAttr(tr, 'aria-rowindex', config.page.row.collection[ids[h]].rowIndex);

												var aoIds2 = [];

												for (var i = 0; i < config.colHeaders.map.length; i++){
													gridInc++;
													var o = config.colHeaders.map[i], cellObject = config.page.row.collection[ids[h]].cells[o.id];
													...
													$A.setAttr(cellObject.cellNode,
																	...
																	'aria-describedby': config.rowHeaders.enabled && config.rowHeaders.id
																		&& config.rowHeaders.id != cellObject.id && cellObject.rowObject.cells[config.rowHeaders.id]
																		? cellObject.rowObject.cells[config.rowHeaders.id].cellNodeId + ' ' + o.thId : o.thId,
																	tabindex: '-1'
																	});
													...
													if (config.page.row.selectEnabled){
														if (cellObject.type == 'toggle'
															&& (config.page.row.toggleButtonRole || config.page.row.toggleButtonState)){
															var altTxt =
																cellObject.value ? config.page.row.toggleButtonRole + ' ' + config.page.row.toggleButtonState
																	: config.page.row.toggleButtonRole;
															$A.setAttr(cellObject.cellNodeA, 'title', altTxt);
														}

														if (config.page.row.ariaSelect)
															$A.setAttr(cellObject.cellNode, 'aria-selected', selected ? 'true' : 'false');
														...
														if (cellObject.type == 'toggle'
															&& (config.page.row.toggleButtonRole || config.page.row.toggleButtonState)){
															altTxt = cellObject.value ? config.page.row.toggleButtonRole + ' ' + config.page.row.toggleButtonState
																: config.page.row.toggleButtonRole;
															altTxt += '. ';
														}
														...
														$A.setAttr(cellObject.cellNodeA, 'title', altTxt + config.page.row.dblClickTitle);
													}

													else if (cellObject.type == 'toggle'
														&& (config.page.row.toggleButtonRole || config.page.row.toggleButtonState)){
														var altTxt = cellObject.value ? config.page.row.toggleButtonRole + ' ' + config.page.row.toggleButtonState
															: config.page.row.toggleButtonRole;
														$A.setAttr(cellObject.cellNodeA, 'title', altTxt);
													}

													if (h === config.page.row.trackPos.row && i === config.page.row.trackPos.col)
														initPos =
																		{
																		rowId: ids[h],
																		cellId: o.id
										...
										prev: function(cellObject, rowObject, byArrow){
											if (config.page.current > 1){
												...
												config.page.row.move(
																{
																rowId: config.page.rendered[byArrow ? config.page.rendered.length - 1 : 0],
										...
										last: function(cellObject, rowObject, lr){
											if (config.page.current < config.page.total){
												...
												config.page.row.move(
																{
																rowId: lr ? config.page.rendered[config.page.rendered.length - 1] : config.page.rendered[0],
																cellId: cellObject.id
																});
		...
		gridInc++;
		var uId = baseId + 'u' + gridInc, getCellObject = function(cellNode){
		...
		config.page.row.editFieldDiv = $A.createEl('div',
						{
						role: 'application'
						}, null, config.editFieldClass);

		config.page.row.editField = $A.createEl('input',
						{
						type: 'text',
		...
		$A.bind(window, 'resize.' + uId, function(){
			if (config.page.row.editFieldActive){
				$A.css(config.page.row.editFieldDiv, config.page.row.cellOffset(config.page.row.editFieldActive));
			}
		});
		...
		$A(
						...
						runAfter: function(dc){
							...
							dc.table = $A.createEl('table',
											{
											role: 'grid',
											id: dc.tableId,
											'aria-owns': dc.theadId + ' ' + dc.tbodyId
											}, null, config.gridClass);
							...
							for (var i = 0; i < config.colHeaders.map.length; i++){
								gridInc++;
								var o = config.colHeaders.map[i], th = $A.createEl('th',
												{
												role: 'columnheader',
								...
								if (o.colClass)
									$A.addClass(th, o.colClass);

								$A.bind(th, 'click', function(ev){
									config.page.row.move();
								});
								th.innerHTML = '<strong id="' + baseId + 'th' + gridInc + 's' + '"><span>' + formatStr(o.name)
									+ '</span></strong>';
		...
		that.changeColumnOrder = function(oldVal, newVal){
			if (isNaN(oldVal) || isNaN(newVal) || !config.colHeaders.map[oldVal] || newVal >= config.colHeaders.map.length)
				return;
			...
			for (var ri = 0; ri < rows.length; ri++){
				...
				if (!(!row || (isNaN(row.id) && !row.id) || !row.cells)){
					if (row.id in config.page.row.collection){
						...
						if (pos !== -1)
							config.page.row.selected.splice(pos, 1);
		...
		that.setSelect = function(o, undefined){
			...
			if ('callback' in o)
				config.page.row.select.cb = typeof o.callback === 'function' ? o.callback : null;
		};
		...
		that.select = function(ids){
			...
			for (var i = 0; i < ids.length; i++){
				var rowId = ids[i], rowObject = config.page.row.collection[rowId];

				if (rowObject && rowObject.cells && rowObject.cells[config.colHeaders.map[0].id]){
					config.page.row.select(rowObject.cells[config.colHeaders.map[0].id], true);
				}
			}
		};
		...
		that.setDelete = function(o){
			...
			if ('callback' in o)
				config.page.row.deleteRow.callback.cb = typeof o.callback === 'function' ? o.callback : null;

			if ('runBefore' in o)
				config.page.row.deleteRow.callback.runBefore = typeof o.runBefore === 'function' ? o.runBefore : null;

			if ('runAfter' in o)
				config.page.row.deleteRow.callback.runAfter = typeof o.runAfter === 'function' ? o.runAfter : null;
		};
		...
		that.deleteAllRows = function(){
			if (config.page.row.deleteRow.enabled){
				return config.page.row.deleteRow.callback(config.page.row.collectionMap);
			}
			return false;
		};
		...
		that.setAccessibleText = function(o){
			...
			if ('disabledText' in o)
				config.page.row.disabledText = o.disabledText;

			if ('editLinkAction' in o)
				config.page.row.editLinkAction = o.editLinkAction;

			if ('pageRole' in o)
				config.page.pageRole = o.pageRole;

			if ('editFieldTitle' in o)
				config.page.row.editFieldTitle = o.editFieldTitle;

			if ('dblClickTitle' in o)
				config.page.row.dblClickTitle = o.dblClickTitle;

			if ('selectState' in o)
				config.page.row.selectState = o.selectState;
		};
		...
		that.setStaticClasses = function(o){
			...
			if ('gridRowClass' in o)
				config.gridRowClass = o.gridRowClass;

			if ('gridCellClass' in o)
				config.gridCellClass = o.gridCellClass;
			...
			if ('editFieldClass' in o)
				config.editFieldClass = o.editFieldClass;
		};
		...
		that.setOpenListener = function(f){
			if (f && typeof f === 'function')
				config.page.on.open = f;
		};

		that.setCloseListener = function(f){
			if (f && typeof f === 'function')
				config.page.on.close = f;
		};

		that.setAddListener = function(f){
			if (f && typeof f === 'function')
				config.page.on.add = f;
		};

		that.setRemoveListener = function(f){
			if (f && typeof f === 'function')
				config.page.on.rem = f;
		};

		that.setMoveListener = function(f){
			if (f && typeof f === 'function')
				config.page.row.move.cb = f;
		};

		that.getData = function(rowId, cellId, keyName){
			if (!rowId || !cellId
				|| !keyName || !(rowId in config.page.row.collection) || !(cellId in config.page.row.collection[rowId].cells))
				return null;
		...
		that.getValue = function(rowId, cellId){
			if (!rowId
				|| !cellId || !(rowId in config.page.row.collection) || !(cellId in config.page.row.collection[rowId].cells))
				return null;
