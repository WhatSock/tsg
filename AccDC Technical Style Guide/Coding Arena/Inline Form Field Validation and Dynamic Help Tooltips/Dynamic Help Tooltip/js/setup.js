    ...
    $A.getEl("questionForm"),
    {
      ...
      questionId: {
        ...
        validate: function(ev, dc) {
          ...
          var a = "terra",
            ...
            pos = (function() {
              ...
              for (var i = 0; i < s.length; i++) {
                if (
                  s.charAt(i) === a.charAt(i) &&
                  s.substring(0, i) === a.substring(0, i)
                )
                  r = i;
              }
              return r;
          ...
          if (s.length === 1 && pos === -1) {
            dc.source =
              "Hmmm, it doesn't look like you're off to a good start.";
            // Now reopen the tooltip so that it renders the new message
            dc.open();
          } else if (s.length === 3 && pos === -1) {
            dc.source =
              "Well, you can keep going if you want, but it won't help much.";
            dc.open();
