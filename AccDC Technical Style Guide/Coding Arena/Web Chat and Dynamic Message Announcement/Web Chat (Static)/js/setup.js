  ...
  var chatObjId = $A.setPopup({
      ...
      runAfter: function(dc) {
        ...
        dc.timer = setTimeout(function() {
          if (!$A.reg[tooltipId].loaded) $A.reg[tooltipId].open();
        }, 3000);
      ...
      runBeforeClose: function(dc) {
        if ($A.reg[tooltipId].loaded) $A.reg[tooltipId].close();
      ...
      submitNew: function(text, name) {
        ...
        div.innerHTML =
          "<strong>" + (name || dc.name) + ":</strong> " + dc.format(text);
    ...
    tooltipId = $A.setTooltip({
      ...
      runAfterClose: function(dc) {
        ...
        if (popup.loaded) {
          ...
          if (i < 12) popup.submitNew(popup.conversation[i], "Blurt");
