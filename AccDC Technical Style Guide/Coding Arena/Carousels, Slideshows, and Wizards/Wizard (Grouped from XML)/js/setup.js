$A.bind(window, "load", function() {
    ...
    $A.setCarousel($A.getEl("carouselId"), "files/carousel.xml", "0,0", {
      ...
      handlers: {
        ...
        complete: function(dc) {
          ...
          if (!dc.groupVal) $A.setAttr(dc.top.btn.P, "disabled", "disabled");
          else $A.remAttr(dc.top.btn.P, "disabled");
          ...
          if (dc.top.saved) {
            for (n in dc.top.saved) {
              if (dc.top.form[n]) dc.top.form[n].value = dc.top.saved[n];
            }
          }
        ...
        btnNext: function(ev, dc) {
          ...
          $A.query('form *[aria-required="true"]', dc.containerDiv, function(
            i,
            o
          ) {
            if (ret && !o.value) {
          ...
          if (!dc.top.saved) dc.top.saved = {};

          // Then save the name / value pairs
          for (var i = 0; i < dc.top.form.elements.length; i++)
            dc.top.saved[dc.top.form.elements[i].name] =
              dc.top.form.elements[i].value;
          ...
          if (dc.top.groupVal == dc.top.groupMax - 1 && dc.top.saved) {
            ...
            for (n in dc.top.saved) s += n + ": " + dc.top.saved[n] + "\n";
            alert(s);
          }
        }
      },
  ...
  $A.bind("#toggleBtn", "click", function(ev) {
    ...
    if (dc && dc.loaded) dc.close();
    // Destroys the AccDC Object automatically to prevent interval conflicts
    else renderCarousel();
  });
});
