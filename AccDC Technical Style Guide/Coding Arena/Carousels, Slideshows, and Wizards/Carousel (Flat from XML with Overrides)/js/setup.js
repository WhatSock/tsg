    ...
    $A.setCarousel($A.getEl("carouselId"), "files/carousel.xml", "0", {
      ...
      renderFn: function(
          ...
          $A.bind(o, {
            ...
            keydown: function(ev) {
              ...
              if (k == 13 || k == 32) {
                ...
                if (k == 37 || k == 38) {
                  if (!dotIndex) dotIndex = dotList.length - 1;
                  else dotIndex--;
                } else if (k == 39 || k == 40) {
                  if (dotIndex >= dotList.length - 1) dotIndex = 0;
                  else dotIndex++;
                }
        ...
        $A.bind(centerDiv, {
          keydown: function(ev) {
            ...
            if (k == 37 || k == 39) {
              if (k == 37) $A.trigger(btnPrev, "click");
              else if (k == 39) $A.trigger(btnNext, "click");
              ev.preventDefault();
            }
          }
        });
      ...
      handlers: {
        ...
        stopStateChange: function(isStopped, dc) {
          ...
          $A[dc.isStopped ? "addClass" : "remClass"](
            stopBtn,
            "stopped"
          ).innerHTML = dc.isStopped
