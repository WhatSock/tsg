$A.bind(window, 'load', function(){
	...
	$A.bind('#rr, #rs, #re', 'click change', function(){
		...
		mode = this.id;
		var mro = $A.getEl('mro'), msc = $A.getEl('msc'), mmm = $A.getEl('mmm');
	...
	var pageHeaderSpan = $A.getEl('pTotal'), pageEdit = $A.getEl('goTo'), pageCurrent = 0, pageTotal = 0,
		...
		lastBtn = $A.getEl('btnLast'), pgn = $A.getEl('pgn');
...
// click and change events are added to normalize across jQuery, Dojo, and MooTools, and a toggle is set to prevent double exicution
		...
		if (isD != nV){
			...
			grid.setDelete(
							...
							runBefore: function(selectedRowIDs_array){
								...
								var i = selectedRowIDs_array.length, s = i === 1 ? '' : 's';

								if (i)
									return confirm('Are you sure you want to delete ' + i + ' row' + s + ' from the grid?');
							...
							runAfter: function(deletedRowIDs_array){
								var i = deletedRowIDs_array.length, s = i === 1 ? '' : 's';
	...
	$A.bind('#bgs', 'click', function(ev){
		...
		var selectedArray = grid.getSelected(), str = 'The following rowObject IDs are currently selected: ';

		if (!selectedArray.length)
			str = 'No rowObjects are currently selected';
